...
pwd;
global root   "`c(pwd)'";
/* global home "${root}\data_proccessing\data_analysis"; *using backslashes instead of forward-slashes */
		...
		{;
			...
			{;
				...
				*gen dummy for top/bottom AOD in 2000;
				...
				bys urbancode: gen nn = _n;
				sum mprewindow_aod if nn==1 & sample_city18 == 1, d;
				gen hiAOD_dummy = ( mprewindow_aod > r(p50) );
				**MW: use continuous AOD;
				gen hiAOD = mprewindow_aod;
				drop prewindow_aod mprewindow_aod nn;
				...
				*generate time dummies for plot;
				...
				if `het' >= 1
				{;
				**MW use high AOD sample;
					keep if hiAOD_dummy == 1 | date_start<tm(2000m2) | date_start==.;
...
**MW: move output;
copy "$gc_data/data_analysis/output_event_study/tables/InteractHighAODTable_s1_10km_18BE_to_18AE_18b_fs.tex" "$tables_mw/table3_ctsaod_hiaod.tex", public replace;
copy "$gc_data/data_analysis/output_event_study/tables/InteractHighAODTable_s1_10km_18BE_to_18AE_18b_fs.csv" "$tables_mw/table3_ctsaod_hiaod.csv", public replace;